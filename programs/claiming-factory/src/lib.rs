...
pub struct Period {
    ...
    /// because it has been claimed outside o  this vesting scope.
    pub airdropped: bool,
}
...
pub struct RefundContract {
    ...
    pub refund_requests: Vec<Pubkey>,
}
...
impl RefundContract {
    pub fn new(admin: Pubkey, refund_deadline: i64) -> Self {
        RefundContract {
            admin,
            refund_deadline,
            refund_requests: vec![],
    ...
    pub fn request_refund(&mut self, user_wallet: Pubkey, current_timestamp: i64) -> Result<()>{
        
        require!(
            current_timestamp > self.refund_deadline,
            VestingAlreadyStarted
        );

        self.refund_requests.push(user_wallet);
        Ok(())
    }

    pub fn has_requested_refund(&self, user_wallet: &Pubkey) -> bool {
        self.refund_requests.contains(user_wallet)
    }

    pub fn remove_refund_request(&mut self, user_wallet: &Pubkey) -> Result<()> {
        if let Some(index) = self.refund_requests.iter().position(|x| *x == *user_wallet) {
            self.refund_requests.remove(index);
        }
        Ok(())
    }
}
